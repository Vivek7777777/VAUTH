...
export default function LoginPage() {
  ...
  async function onSubmit(values: z.infer<typeof loginSchema>) {
    ...
    try {
      ...
      if (signInError) {
        setServerError(signInError.message || "Invalid credentials. Please try again.")
      } else {
        router.push("/")
      }
    } catch (err) {
      setServerError("A server error occurred. Please contact support.")
    } finally {
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-2 text-center">
        ...
        <p className="text-zinc-400 font-medium">Enter your credentials to access your account</p>
      </div>
              ...
              render={({ field }) => (
                <FormItem>
                  <div className="flex items-center justify-between">
                    ...
                    <Link href="/forgot-password" className="text-xs text-zinc-500 hover:text-black transition-colors font-medium">
                      Forgot password?
                    </Link>
                  </div>
                  <FormControl>
